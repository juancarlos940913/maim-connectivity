...
static device_value_entry_t states[
    DEVICE_MANAGER_MAX_STATES
];

static device_value_entry_t metrics[
    DEVICE_MANAGER_MAX_METRICS
];

static device_value_entry_t sensors[
    DEVICE_MANAGER_MAX_SENSORS
];

static device_value_entry_t outputs[
    DEVICE_MANAGER_MAX_OUTPUTS
];

static device_error_entry_t errors[
    DEVICE_MANAGER_MAX_ERRORS
];
...
static device_manager_event_callback_t
    event_callback = NULL;

static device_manager_transaction_callback_t
...
static const char *get_value(
    const device_value_entry_t *table,
    size_t table_size,
    const char *key
);

static esp_err_t add_error(
    const char *code,
    const char *severity
);

static esp_err_t clear_error(
    const char *code
);

static bool severity_is_valid(
    const char *severity
);

static esp_err_t parse_transaction_id(
    const char *text,
...
)
{
    ...
    if (
        strlen(key) >= DEVICE_MANAGER_KEY_LEN ||
...
)
{
    ...
    for (
        size_t i = 0;
        i < DEVICE_MANAGER_MAX_ERRORS;
        i++
    )
    {
        if (
            errors[i].used &&
            strcmp(errors[i].code, code) == 0
        )
        {
            strncpy(
                errors[i].severity,
                severity,
                sizeof(errors[i].severity) - 1
            );
    ...
    switch (frame->type)
    {
        ...
        case UART_FRAME_HELLO:
        {
            if (frame->field_count != 4)
            {
                ESP_LOGW(
                    TAG,
                    "HELLO invalido. Se esperaban 4 campos"
                );
        ...
        case UART_FRAME_METRIC:
        {
            if (frame->field_count != 2)
            {
                ESP_LOGW(
                    TAG,
                    "METRIC invalido. Se esperaban KEY,VALUE"
                );
        ...
        case UART_FRAME_SENSOR:
        {
            if (frame->field_count != 2)
            {
                ESP_LOGW(
                    TAG,
                    "SENSOR invalido. Se esperaban KEY,VALUE"
                );
        ...
        case UART_FRAME_OUTPUT:
        {
            if (frame->field_count != 2)
            {
                ESP_LOGW(
                    TAG,
                    "OUTPUT invalido. Se esperaban KEY,VALUE"
                );
        ...
        case UART_FRAME_ERROR:
        {
            if (frame->field_count != 2)
            {
                ESP_LOGW(
                    TAG,
                    "ERROR invalido. Se esperaban CODE,SEVERITY"
                );
        ...
        case UART_FRAME_SNAPSHOT:
        {
            ...
            )
            {
                ...
                )
                {
                    ESP_LOGW(
                        TAG,
                        "SNAPSHOT END no corresponde al activo"
                    );
            ...
            else
            {
                ESP_LOGW(
                    TAG,
                    "Operacion SNAPSHOT desconocida: %s",
                    frame->fields[0]
                );
        ...
        case UART_FRAME_NACK:
        {
            if (frame->field_count < 1)
            {
                ESP_LOGW(
                    TAG,
                    "Respuesta de transaccion sin ID"
                );
        ...
        case UART_FRAME_CONFIG:
        {
            if (frame->field_count != 2)
            {
                ESP_LOGW(
                    TAG,
                    "CONFIG invalido. Se esperaban KEY,VALUE"
                );
...
void device_manager_print_status(void)
{
    ...
    else
    {
        ...
        {
            const device_error_entry_t *error =
                device_manager_get_error(i);
