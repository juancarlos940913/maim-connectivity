...
typedef struct
{
    ...
    char mqtt_command_id[
        TRANSACTION_MANAGER_COMMAND_ID_LEN
    ];

    char command[
        TRANSACTION_MANAGER_COMMAND_LEN
    ];
...
static transaction_entry_t transactions[
    TRANSACTION_MANAGER_MAX_ACTIVE
];

static uint16_t next_uart_id = 1;

static transaction_manager_result_callback_t
...
static transaction_entry_t *
find_transaction(uint16_t uart_id)
{
    for (
        size_t i = 0;
        i < TRANSACTION_MANAGER_MAX_ACTIVE;
        i++
    )
    {
        if (
            transactions[i].used &&
            transactions[i].uart_id == uart_id
        )
        {
...
static void release_transaction(
    transaction_entry_t *transaction
)
{
    if (transaction == NULL)
    {
...
void transaction_manager_init(void)
{
    ...
    ESP_LOGI(
        TAG,
        "Transaction Manager inicializado"
    );
}
...
)
{
    ...
    if (transaction == NULL)
    {
        ESP_LOGE(
            TAG,
            "No hay espacio para nueva transaccion"
        );
    ...
    strncpy(
        transaction->command,
        command,
        sizeof(transaction->command) - 1
    );

    transaction->state =
        TRANSACTION_STATE_WAITING_ACK;

    transaction->created_us =
    esp_timer_get_time();
    ...
    *uart_transaction_id = id;
    ...
    ESP_LOGI(
        TAG,
        "Esperando ACK del controlador | UART ID=%u | MQTT ID=%s",
        id,
        mqtt_command_id
    );
...
if (type == UART_FRAME_ACK)
{
    ...
    )
    {
        ESP_LOGW(
            TAG,
            "ACK duplicado ignorado | UART ID=%u",
            transaction_id
        );
...
void transaction_manager_process_timeouts(void)
{
    ...
    const int64_t ack_timeout_us =
        (int64_t)TRANSACTION_ACK_TIMEOUT_MS * 1000LL;
